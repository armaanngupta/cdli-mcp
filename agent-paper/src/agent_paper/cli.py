...
async def run(topic: str, filters: dict[str, str], provider: str, key: str, model: str):
    ...
    async for kind, payload, state in stream_run(topic, filters, provider, key, model):
        if kind == "section":
            print(f"[section   ] {payload['index']}/{payload['of']} {payload['section']}")
            continue
        described = " ".join(f"{k}={v}" for k, v in payload["progress"].items())
        print(f"[{payload['name']:<10}] {described}")
    return state
