...
export function classifyError(raw: string): FriendlyError {
  ...
  if (has('no artifacts found')) {
  ...
  } else if (has('401', 'unauthorized', 'invalid api key', 'incorrect api key', 'invalid_api_key')) {
    message = 'That API key was rejected. Check the key for your selected provider and re-enter it.';
    ...
    has('unreachable', 'fetch failed', 'failed to fetch', 'econnrefused', '502', '503', 'bad gateway')
  ) {
  ...
  } else if (has('did not call a tool', 'tool_use_failed', 'tool choice')) {
